...
export default function RiderDeliveryScreen() {
 const [activeOrder, setActiveOrder] = useState<DeliveryOrder>(mockActiveOrder)
 const [pastDeliveries] = useState<PastDelivery[]>(mockPastDeliveries)
 ...
 const updateOrderStatus = (newStatus: DeliveryOrder["status"]) => {
	...
	if (newStatus === "delivered") {
	 Alert.alert("Delivery Complete!", "Great job! The package has been delivered successfully.", [
 ...
 const handleNavigate = (coords: { latitude: number; longitude: number }, label: string) => {
				...
				{index < statuses.length - 1 && (
					<View
						style={[
						 styles.statusLine,
						 {
							backgroundColor: index < currentIndex ? getStatusColor(statuses[index + 1]) : "#e5e7eb",
						 },
						]}
					/>
				)}
 ...
 const generateMapHTML = () => {
    ...
    <body>
        ...
        <script>
            // Initialize map
            var map = L.map('map').setView([${(pickupCoords.latitude + dropoffCoords.latitude) / 2}, ${(pickupCoords.longitude + dropoffCoords.longitude) / 2}], 12);
            ...
            var pickupMarker = L.marker([${pickupCoords.latitude}, ${pickupCoords.longitude}], {icon: pickupIcon})
                .addTo(map)
                .bindPopup('<b>Pickup Location</b><br>${activeOrder.pickupLocation}');
                
            var dropoffMarker = L.marker([${dropoffCoords.latitude}, ${dropoffCoords.longitude}], {icon: dropoffIcon})
                .addTo(map)
                .bindPopup('<b>Drop-off Location</b><br>${activeOrder.dropoffLocation}');
		...
		<ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
		 ...
		 >
			...
			<View style={styles.customerInfo}>
			 ...
			 <TouchableOpacity style={styles.callButton} onPress={() => handleCall(activeOrder.customerPhone)}>
			...
			<View style={styles.locationInfo}>
			 <View style={styles.locationRow}>
				...
				<View style={styles.locationDetails}>
				 ...
				 <Text style={styles.locationAddress}>{activeOrder.pickupLocation}</Text>
				</View>
			 </View>
			 ...
			 <View style={styles.locationRow}>
				...
				<View style={styles.locationDetails}>
				 ...
				 <Text style={styles.locationAddress}>{activeOrder.dropoffLocation}</Text>
				</View>
			 </View>
			</View>
			...
			<View style={styles.packageInfo}>
			 ...
			 {activeOrder.specialInstructions && (
				 <Text style={styles.specialInstructions}>💬 {activeOrder.specialInstructions}</Text>
			 )}
			</View>
			...
			<View style={styles.deliveryStats}>
			 ...
			 <View style={styles.statItem}>
				<Text style={styles.statValue}>₦{activeOrder.earning.toLocaleString()}</Text>
		 ...
		 {activeOrder.status !== "delivered" && (
			 <TouchableOpacity
				 style={[styles.actionButton, { backgroundColor: getStatusColor(activeOrder.status) }]}
				 onPress={() => updateOrderStatus(getNextStatus(activeOrder.status) as DeliveryOrder["status"])}
			 >
				<Text style={styles.actionButtonText}>{getNextStatusText(activeOrder.status)}</Text>
			 </TouchableOpacity>
		 )}

		 {/* Past Deliveries */}
		 <Animated.View style={[styles.pastDeliveriesCard, { opacity: fadeAnim }]}>
			<View style={styles.pastDeliveriesHeader}>
			...
			{pastDeliveries.map((delivery) => (
				<View key={delivery.id} style={styles.pastDeliveryItem}>
				 <View style={styles.pastDeliveryInfo}>
					<Text style={styles.pastCustomerName}>{delivery.customerName}</Text>
					<Text style={styles.pastLocation}>{delivery.location}</Text>
					<Text style={styles.pastTime}>{delivery.completedAt}</Text>
				 </View>
				 <View style={styles.pastDeliveryStats}>
					<Text style={styles.pastEarning}>₦{delivery.earning.toLocaleString()}</Text>
					<View style={styles.ratingContainer}>
					 <Text style={styles.ratingStars}>{"⭐".repeat(delivery.rating)}</Text>
					</View>
				 </View>
				</View>
			))}
