...
const demoDeliveryRequests: DeliveryRequest[] = [
 {
	id: "req_001",
...
export default function RiderHomeScreen() {
 ...
 const [documentsUploaded] = useState(true) // Demo: set to false to show caution
 ...
 useEffect(() => {
	setTimeout(() => {
	 ...
	 setActiveOrder(
		 demoPendingOrders.find((order) => order.status === "accepted" || order.status === "in_transit") || null,
	 )
	 setIsLoading(false)
	}, 1500)
 }, [])
 ...
 useEffect(() => {
	if (isOnline) {
	 setTimeout(() => {
		setCurrentRequest(demoDeliveryRequests[0])
 ...
 const handleAcceptRequest = () => {
	if (currentRequest) {
	 const newOrder: PendingOrder = {
		id: `ord_${Date.now()}`,
		orderNumber: `TZ${Math.floor(Math.random() * 1000000)}`,
		customerName: currentRequest.customerName,
		pickupLocation: currentRequest.pickupLocation,
		dropoffLocation: currentRequest.dropoffLocation,
		status: "accepted",
		estimatedTime: "25 min",
		earning: currentRequest.estimatedEarning,
 ...
 const handleDeclineRequest = () => {
	...
	setTimeout(() => {
	 if (isOnline && demoDeliveryRequests[1]) {
		setCurrentRequest(demoDeliveryRequests[1])
		setShowDeliveryModal(true)
		fadeAnim.setValue(0)
		Animated.timing(fadeAnim, {
		 toValue: 1,
 ...
 const paginatedOrders = pendingOrders.slice(currentPage * ordersPerPage, (currentPage + 1) * ordersPerPage)
		...
		<ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
		 ...
		 <View style={styles.statusCard}>
			<View style={styles.statusHeader}>
			 <View style={styles.statusInfo}>
				...
				<Text style={[styles.statusText, { color: isOnline ? "#00B624" : "#ef4444" }]}>
		 ...
		 {!documentsUploaded && (
			 <View style={styles.cautionCard}>
				...
				<Text style={styles.cautionText}>
				 You can&#39;t receive orders until you&#39;ve uploaded your required documents (driver&#39;s license, insurance, etc.)
				</Text>
				...
				<View style={styles.activeOrderCard}>
				 <View style={styles.orderHeader}>
					...
					<View style={styles.orderStatus}>
					 <Text style={styles.statusIcon}>{getStatusIcon(activeOrder.status)}</Text>
					 <Text style={[styles.statusLabel, { color: getStatusColor(activeOrder.status) }]}>
						{activeOrder.status.replace("_", " ")}
					 </Text>
					</View>
				 </View>
				 <View style={styles.orderDetails}>
					<Text style={styles.locationText}>📍 {activeOrder.pickupLocation}</Text>
					<Text style={styles.locationText}>📍 {activeOrder.dropoffLocation}</Text>
				 </View>
				 <View style={styles.orderFooter}>
					<Text style={styles.orderDistance}>
					 {activeOrder.distance} • {activeOrder.estimatedTime}
					</Text>
					<Text style={styles.orderEarning}>₦{activeOrder.earning.toLocaleString()}</Text>
				 </View>
				</View>
			 </View>
		 )}
		 ...
		 <View style={styles.statsContainer}>
			<View style={styles.statCard}>
			 <Text style={styles.statValue}>₦{riderStats.todayEarnings.toLocaleString()}</Text>
				 ...
				 {paginatedOrders.map((order) => (
					 <TouchableOpacity key={order.id} style={styles.orderCard}>
						...
						<View style={styles.orderDetails}>
						 ...
						 <Text style={styles.locationText}>📍 {order.dropoffLocation}</Text>
						</View>
						<View style={styles.orderFooter}>
						 <Text style={styles.orderDistance}>
							{order.distance} • {order.estimatedTime}
						 </Text>
						 <Text style={styles.orderEarning}>₦{order.earning.toLocaleString()}</Text>
						</View>
					 </TouchableOpacity>
				 ))}
			...
			<Animated.View style={[styles.modalContent, { opacity: fadeAnim }]}>
			 <View style={styles.modalHeader}>
				...
				<View
					style={[
					 styles.urgencyBadge,
					 { backgroundColor: currentRequest?.urgency === "urgent" ? "#fef3c7" : "#f0f9ff" },
					]}
				>
				 <Text
					 style={[styles.urgencyText, { color: currentRequest?.urgency === "urgent" ? "#d97706" : "#0369a1" }]}
				 >
			 ...
			 {currentRequest ? (
				 <>
					<View style={styles.requestDetails}>
					 <Text style={styles.modalCustomerName}>{currentRequest.customerName}</Text>
					 <Text style={styles.packageType}>{currentRequest.packageType}</Text>
					 <Text style={styles.timeAgo}>{currentRequest.timeAgo}</Text>
					</View>

					<View style={styles.locationDetails}>
					 <Text style={styles.locationLabel}>Pickup</Text>
					 <Text style={styles.modalLocationText}>📍 {currentRequest.pickupLocation}</Text>
					 <Text style={styles.locationLabel}>Drop-off</Text>
					 <Text style={styles.modalLocationText}>📍 {currentRequest.dropoffLocation}</Text>
					</View>

					<View style={styles.requestFooter}>
					 <Text style={styles.distanceText}>{currentRequest.distance}</Text>
					 <Text style={styles.earningText}>₦{currentRequest.estimatedEarning.toLocaleString()}</Text>
					</View>
				 ...
				 <View style={styles.modalEmptyState}>
					<Text style={styles.modalEmptyText}>Loading request...</Text>
