...
const generateTransactions = (page = 0, limit = 20): Transaction[] => {
 ...
 for (let i = 0; i < limit; i++) {
	...
	const transaction: Transaction = {
	 ...
	 id: `txn_${page}_${i}`,
	 date: new Date(Date.now() - (page * limit + i) * 24 * 60 * 60 * 1000).toISOString(),
	}
...
export default function TransactionHistoryScreen(): JSX.Element {
 ...
 const TransactionItem = ({ transaction }: { transaction: Transaction }): JSX.Element => (
	 <TouchableOpacity
	 ...
	 >
		...
		<View style={styles.transactionRight}>
		 <Text
			 style={[styles.transactionAmount, transaction.amount > 0 ? styles.positiveAmount : styles.negativeAmount]}
		 >
 ...
 const FilterButton = ({ type, label }: { type: "all" | TransactionType; label: string }): JSX.Element => (
	 <TouchableOpacity
	 ...
	 <SafeAreaView style={styles.container}>
		...
		{ loading ?
			...
			<FlatList<Transaction>
				 ...
				 refreshControl={
					<RefreshControl refreshing={ refreshing } onRefresh={ onRefresh } colors={ [tzColors.primary] } tintColor={tzColors.primary} />
				 }
